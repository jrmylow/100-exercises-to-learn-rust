...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            Err(TicketNewError::TitleError{msg: "Title cannot be empty".to_string()})
        } else if title.len() > 50 {
            Err(TicketNewError::TitleError{msg: "Title cannot be longer than 50 bytes".to_string()})
        } else if description.is_empty() {
            Err(TicketNewError::DescriptionError{msg: "Description cannot be empty".to_string()})
        } else if description.len() > 500 {
            Err(TicketNewError::DescriptionError{msg: "Description cannot be longer than 500 bytes".to_string()})
        } else {
            Ok(Ticket {
                title,
                description,
